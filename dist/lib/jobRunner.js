...
const DOMAINS = ["http", "infra", "auth"];
export async function runJob(windowMinutes = 60) {
    const jobId = uuid();
    const windowTo = Date.now() * 1000000;
    const windowFrom = windowTo - (24 * windowMinutes * 60 * 1000000000);
    console.log(`\n🚀 [JOB START] ${jobId}`);
    ...
    for (const domain of DOMAINS) {
        const lines = await fetchLogs(domain, windowFrom, windowTo).catch(() => []);
        logs[domain] = lines;
    ...
    for (const step of remediationSteps) {
        ...
        try {
            const finding = findings.find((f) => f.finding_id === step.findingId);
...
function buildRemediationSteps(findings) {
    const steps = [];
    for (const f of findings) {
        const recommendation = `${f.classification} ${f.summary} ${f.recommended_action || ""}`
            .toLowerCase();
        // BLOCK IP cases
        if (recommendation.includes("block ip") ||
        ...
        // RATE LIMIT cases
        if (recommendation.includes("rate limit") ||
        ...
        if (recommendation.includes("increase memory") ||
...
// }
async function executeAction(step, finding) {
    console.log("executing actions s s");
    switch (step.actionType) {
        case "block_ip":
            console.log(`🔥 Blocking IP`);
            await blockIpOnCloudflare(finding?.offender?.value, `Blocked due to ${finding?.classification}`);
            break;
        case "rate_limit":
            console.log(`🚦 Applying rate limit`);
            await createJiraTicket(`rate-limit-${step.findingId}`, [finding]);
            break;
        case "allocate_resources":
            console.log(`📈 Allocating resources`);
            await createJiraTicket(`infra-${step.findingId}`, [finding]);
            break;
        case "alert_soc":
            console.log(`🚨 Alerting SOC`);
            await notifySlack(`
Job ID: ${finding?.finding_id}`, `Summary: ${finding?.summary}`, `Classification: ${finding?.classification}`, `Severity: ${finding?.severity}`, `Recommended Action: ${finding?.recommended_action}`);
            break;
        default:
